...
if [[ $(docker volume ls -q --filter name=dispatch-postgres) && $(docker run --rm -v dispatch-postgres:/db busybox cat /db/PG_VERSION 2>/dev/null) == "9.5" ]]; then
    ...
    # If this is Postgres 9.5 data, start upgrading it to 11 in a new volume
    docker run --rm \
    -v dispatch-postgres:/var/lib/postgresql/9.5/data \
    -v dispatch-postgres-new:/var/lib/postgresql/11/data \
    tianon/postgres-upgrade:9.5-to-11
    ...
    # Also append the `host all all all trust` line as `tianon/postgres-upgrade:9.5-to-11`
